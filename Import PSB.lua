...
local function importFromPsd(filename)
    ...
    if not file then
        return false, "Failed to open PSB file: " .. filename
    end
    ...
    if signature ~= "8BPS" then
        file:close()
        return false, "Not a valid PSB file (invalid signature)"
    end
    ...
    if version ~= 1 then
        file:close()
        return false, "Unsupported PSB version: " .. version
    end
        ...
        if filename and filename ~= "" then
            ...
            if success then
                app.alert({
                    ...
                    text = "PSB file imported successfully!\n" .. filename,
                    buttons = "OK"
                })
            else
                app.alert({
                    title = "Import Failed",
                    text = "An error occurred while importing the PSB file:\n" .. (errorMessage or "Unknown error"),
                    buttons = "OK"
                })
            end
        end
...
if app.isUIAvailable then
    ...
    if filename then
        local success, errorMessage = importFromPsd(filename)
        if success then
            print("PSB file imported successfully: " .. filename)
        else
            io.stderr:write("Import failed: " .. (errorMessage or "Unknown error"))
        end
    else
        io.stderr:write("Usage: aseprite -b -script 'import from psd.lua' --filename=file.psb")
    end
end
